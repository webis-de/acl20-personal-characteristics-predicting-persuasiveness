...
com_file = sc.textFile('/corpora/corpora-thirdparty/corpora-reddit/corpus-submissions/')
...
com_processed = com_json.map(lambda s : urls(s, 'body'))
com_processed.cache()

com_processed = com_processed.map(lambda l : json.dumps(l, ensure_ascii=False).encode('utf8'))
com_processed.saveAsTextFile("hdfs://hadoop:8020/user/username/data/cmv_sub_urls")